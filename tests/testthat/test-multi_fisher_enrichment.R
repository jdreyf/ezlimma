...
test_that("non-sim multi fisher", {
  ...
  fe <- fisher_enrichment(sig.set = sig.set, G=G, feat.tab = eztt)
  fe <- fe[order(fe$top.p),]
  
  ss2 <- list(bottom=rownames(eztt.o)[51:100])
  fe2 <- fisher_enrichment(sig.set = ss2, G=G, feat.tab = eztt)
  
  mfe <- multi_fisher_enrichment(sig.sets = c(sig.set, ss2), G=G, feat.tab = eztt[, 1:3])
  ...
  expect_equal(mfe[[2]][, "top"], as.numeric(rownames(mfe[[2]]) %in% sig.set[[1]]))
})