...
## Simulate data
I'll simulate log2 expression data with 100 genes and 6 samples in two groups, with the first gene up-regulated in the first group. This is the example data from  \code{\link[limma]{contrasts.fit}}.
```{r sim}
set.seed(42)
...
If we set *name* to a value other than `NA`, this would creates a folder with one CSV per pathway, and an Excel file that looks like
```{r, echo=FALSE}
knitr::kable(res5)
```
