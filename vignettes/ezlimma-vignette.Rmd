...
pheno <- data.frame(grp=rep(c("First3", "Last3"), each=3), yy=rnorm(ncol(M)))
...
```{r}
grp.num <- batch2design(pheno$grp)
res.hit <- hitman(E = grp.num, M = M, Y = setNames(pheno$yy, nm=colnames(M)))
```
...
```{r}
res5 <- roast_contrasts(object=M, G=G, stats.tab=res0, grp=pheno$grp, contrast.v = contr.v, fun="fry", name=NA)
```
