...
```{r}
grp.num <- batch2design(grp)
res.hit <- hitman(E = grp.num, M = M, Y = setNames(pheno.v, nm=colnames(M)))
```
...
```{r}
res5 <- roast_contrasts(object=M, G=G, stats.tab=res0, grp=grp, contrast.v = contr.v, fun="fry", name=NA)
```
